    ...
    useEffect(() => {
        const loadPromoConfig = async () => {
            try {
                ...
                const res = await fetch('https://staging.stiles.co.za/api/admin-unique-promos.php', {
                    method: 'GET',
                    headers: {
...
const Content = ({
    ...
    useEffect(() => {
        const fetchFilterValues = async () => {
            try {
                const response = await fetch(`https://staging.stiles.co.za/api/products.php?promo=${encodeURIComponent(promoName)}&filters=true`);
                if (!response.ok) {
