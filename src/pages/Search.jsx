...
const Content = ({
    ...
    useEffect(() => {
        ...
        const fetchFilterValues = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/search.php?q=${searchQuery}&filters=true`);
                if (!response.ok) {
    ...
    useEffect(() => {
        ...
        const fetchProducts = async () => {
            try {
                ...
                const requestUrl = `${import.meta.env.VITE_API_BASE_URL}/api/search.php?${queryParams.toString()}`;
