...
const ProductCard = memo(({ onClick, prod }) => {
    ...
    useEffect(() => {
        ...
        fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products.php?slug=${prod}`)
        .then(async res => {
            if (!res.ok) {
        ...
        .then(data => {
            ...
            if (data.status === 'success' && data.data) {
                ...
                if (productData.sku) {
                    fetch(`${import.meta.env.VITE_API_BASE_URL}/api/iq_new.php?code=${productData.sku}`, {
                        method: 'GET',
                        headers: {
        ...
        >
            ...
            >
                {(() => {
                    ...
                    // For sanitary-ware and slabs: only show when onhand === 0
